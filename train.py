...
class ChartEncoder(nn.Module):
    def __init__(self, vocab_size):
        ...
        self.compose = nn.Linear(2*self.hidden_size, self.hidden_size)
...
def main(options):
    ...
    model = ChartEncoder(vocab_size=len(tr_dataset['metadata']['word2idx']))
    loss_func = ConstituentLoss(vocab_size=len(tr_dataset['metadata']['label2idx']))
    container = ModelContainer(model, loss_func)
    ...
    for epoch in range(options.max_epochs):
        for batch_map in batch_iterator.get_iterator():
            x = BatchManager().prepare_batch(batch_map)
